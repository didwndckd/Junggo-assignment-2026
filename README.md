...
원격 상품 이미지는 `Kingfisher`로 로드했습니다. 비동기 이미지 다운로드, 메모리·디스크 캐싱, 로딩·실패 상태 처리를 직접 구현하는 대신 화면 코드에서 간결하게 처리하기 위함입니다.

과제 규모에서는 `AsyncImage`도 선택지였지만, 이미지 캐시 정책과 재사용을 명확히 가져가기 위해 Kingfisher를 사용했습니다.

### 개선하고 싶은 부분

#### 목록 보기 방식 저장

현재 1열·2열 보기 방식은 화면의 `@State`로만 관리되어 앱을 다시 실행하면 기본값으로 돌아갑니다. `AppStorage`를 사용해 사용자가 마지막으로 선택한 보기 방식을 저장하면, 다음 실행에서도 동일한 목록 경험을 제공할 수 있습니다.

#### 목록 페이징과 데이터 캐시

요구사항은 단일 목록 조회를 기준으로 했고, 1일 과제에서는 목록·상세·찜 상태 동기화 등 필수 흐름의 완성도에 집중하기 위해 페이징을 범위에서 제외했습니다.

실제 상품 수가 늘어나는 환경에서는 `limit`·`skip` 기반의 페이징과 스크롤 하단 추가 로딩을 적용하고, 상품 목록 응답 캐시와 만료 정책을 추가해 초기 로딩 시간과 네트워크 요청을 줄이고 싶습니다.
...
- 자체 제작한 `ios-project-builder` 플러그인을 사용해 요구사항을 정리하고, 네트워크 베이스를 포함한 프로젝트 초기 구조를 구성했습니다.
- 프로젝트 규칙을 문서화하기 위해 `CLAUDE.md`와 rule 파일 작성에 활용했습니다.
- API 공식 문서 탐색 및 요청·응답 형식 검증에 활용했습니다.
- 구현 과정에서는 페어 코딩 방식으로 설계·코드·테스트 방향을 함께 검토했습니다.
- 커밋 메시지와 PR 설명 초안 작성에 활용했습니다.

#### AI 결과 반영 방식

요구사항을 프롬프트로 전달한 뒤 결과를 확인하고 반영했습니다. 요구사항이 부족하거나 의도와 다른 부분은 추가 프롬프트로 보완했으며, 필요한 경우 직접 코드를 수정했습니다. AI가 생성한 결과를 검토 없이 그대로 적용하지 않았습니다.

#### 검증 방식

이해하고 있는 구현·설계 내용은 직접 코드와 테스트를 확인한 뒤 반영했습니다. API 관련 내용은 공식 문서를 확인하거나 `curl` 요청으로 실제 응답을 검증했습니다. AI 제안은 요구사항 충족 여부와 기존 코드 구조·동작에 부합하는지를 기준으로 검토했습니다.
